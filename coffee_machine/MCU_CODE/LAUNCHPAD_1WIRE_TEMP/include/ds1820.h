...
* RETURN:     - 
******************************************************************************/ 
//#define delay_us(dly_us)       delay_us(dly_us)

void delay_us(uint32 dly_us)
{
	int i=0;
	for(i=0;i<27*dly_us;i++)
	;
}
/******************************************************************************* 
...
void delay_ms(uint32 dly_ms)
{
	int i=0;
	for(i=0;i<27000*dly_ms;i++)
	;
}
...
#define DS1820_DisableInterrupts()  _disable_interrupt() 
#else 
...
******************************************************************************/ 
#ifdef DS1820_INTERRUPT_LOCK 
#define DS1820_EnableInterrupts()   _enable_interrupt() 
#else 
...
{ 
  ...
  /* reset pulse */ 
  gioSetBit(gioPORTA,DS1820_DATAPIN,0); 
  delay_us(DS1820_RST_PULSE);
  gioSetBit(gioPORTA,DS1820_DATAPIN,1);

  /* wait until pullup pull 1-wire bus to high */ 
  delay_us(DS1820_PRESENCE_WAIT);

  /* get presence pulse */ 
  bPresPulse = gioGetBit(gioPORTA,DS1820_DATAPIN);
  printf("(bool)bPresPulse = %d\n\r\0",bPresPulse);
  delay_us(424);
...
{ 
  ...
  gioGetBit(gioPORTA,DS1820_DATAPIN);
  delay_us(DS1820_BITREAD_DLY);

  bBit = (bool)gioGetBit(gioPORTA,DS1820_DATAPIN);
...
* RETURN:     - 
******************************************************************************/ 
void DS1820_WriteBit(uint8 bBit)
{ 
  printf("in write bit \n\r\0");
  DS1820_DisableInterrupts(); 
  ...
  delay_us(DS1820_BITWRITE_DLY);
  gioSetBit(gioPORTA,DS1820_DATAPIN,1);
  printf("write bit pass\n\r\0");
  DS1820_EnableInterrupts(); 
} 
